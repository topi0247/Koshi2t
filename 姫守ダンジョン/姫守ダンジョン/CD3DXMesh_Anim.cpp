...
HRESULT MyHierarchy::CreateFrame(LPCSTR Name, LPD3DXFRAME *ppNewFrame)
{
	...
	*ppNewFrame = NULL;
	...
	strcpy_s(pFrame->Name, sizeof(pFrame->Name),Name);
...
//
//HRESULT MyHierarchy::CreateMeshContainer
//メッシュコンテナーを作成する
HRESULT MyHierarchy::CreateMeshContainer(LPCSTR Name, CONST D3DXMESHDATA* pMeshData,
				...
				LPD3DXMESHCONTAINER *ppMeshContainer) 
{
	...
	*ppMeshContainer = NULL;
	...
	strcpy_s(pMeshContainer->Name, sizeof(pMeshContainer->Name), Name);
	...
	if (NumMaterials > 0)
	{
		...
		for (iMaterial = 0; iMaterial < NumMaterials; iMaterial++)
		{
			if (pMaterials[iMaterial].pTextureFilename != NULL)
			{
				...
				strcpy_s(pMeshContainer->pMaterials[iMaterial].pTextureFilename, sizeof(pMeshContainer->pMaterials[iMaterial].pTextureFilename), pMaterials[iMaterial].pTextureFilename);
			}
		}
	}
	else
...
HRESULT CD3DXMesh_Anim::CreateAppMeshFromD3DXMesh(LPD3DXFRAME p)
{
	...
	pAppMesh->pMaterial = new MyMaterial[pAppMesh->dwNumMaterial];
	...
	for( DWORD i=0; i<pAppMesh->dwNumMaterial; i++ )
	{
			...
			lstrlenA(pFrame->pMeshContainer->pMaterials[i].pTextureFilename) > 0 )
		{
			...
			strcpy_s(pAppMesh->pMaterial[i].szTextureName,sizeof(pAppMesh->pMaterial[i].szTextureName), pFrame->pMeshContainer->pMaterials[i].pTextureFilename);
			//テクスチャーを作成
	...
	BYTE *pVertices=NULL;
	MyVertexNotex* pvVertex=NULL;
	MyVertexTex* pvVertexTex=NULL;
	...
	{		
		...
		if(!pAppMesh->Tex)
		{
			pvVertex=(MyVertexNotex*)pVertices;
			bd.ByteWidth = sizeof( MyVertexNotex ) *pD3DXMesh->GetNumVertices();
			InitData.pSysMem = pvVertex;
		}
		else
		{
			pvVertexTex=(MyVertexTex*)pVertices;
			bd.ByteWidth = sizeof( MyVertexTex ) *pD3DXMesh->GetNumVertices();
...
HRESULT CD3DXMesh_Anim::InitShader()
{
	...
	cb.ByteWidth= sizeof( BufferPerMesh );
	...
	cb.BindFlags= D3D11_BIND_CONSTANT_BUFFER;
	cb.ByteWidth= sizeof( BufferPerMaterial );
	cb.CPUAccessFlags=D3D11_CPU_ACCESS_WRITE;
	cb.MiscFlags	=0;
	cb.Usage=D3D11_USAGE_DYNAMIC;
...
//
//
void CD3DXMesh_Anim::DrawPartsMesh(PartsMesh* pPartsMesh,D3DXMATRIX World)
{
	...
	if( SUCCEEDED( m_pDeviceContext11->Map( m_pConstantBuffer0, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		memcpy_s( pData.pData, pData.RowPitch, (void*)&sg, sizeof( BufferPerMesh ) );
	...
	for(DWORD i=0;i<pPartsMesh->dwNumMaterial;i++)
	{
		...
		if( SUCCEEDED( m_pDeviceContext11->Map( m_pConstantBuffer1,0, D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
		{
			...
			memcpy_s( pData.pData, pData.RowPitch, (void*)&sg, sizeof( BufferPerMaterial ) );
