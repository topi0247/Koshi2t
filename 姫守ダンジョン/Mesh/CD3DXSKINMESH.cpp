...
D3DXMATRIX CD3DXSKINMESH::m_View;
D3DXMATRIX CD3DXSKINMESH::m_Proj;
HWND CD3DXSKINMESH::m_hWnd = nullptr;
LPDIRECT3D9 CD3DXSKINMESH::m_pD3d9 = nullptr;
LPDIRECT3DDEVICE9 CD3DXSKINMESH::m_pDevice9 = nullptr;
ID3D11Device* CD3DXSKINMESH::m_pDevice = nullptr;
ID3D11DeviceContext* CD3DXSKINMESH::m_pDeviceContext = nullptr;
ID3D11VertexShader* CD3DXSKINMESH::m_pVertexShader = nullptr;
ID3D11PixelShader* CD3DXSKINMESH::m_pPixelShader = nullptr;
ID3D11InputLayout* CD3DXSKINMESH::m_pVertexLayout = nullptr;
ID3D11Buffer* CD3DXSKINMESH::m_pConstantBuffer0 = nullptr;
ID3D11Buffer* CD3DXSKINMESH::m_pConstantBuffer1 = nullptr;
ID3D11SamplerState* CD3DXSKINMESH::m_pSampleLinear = nullptr;
ID3D11Buffer* CD3DXSKINMESH::m_pConstantBufferBone = nullptr;
...
{
	if (m_pContainer->pSkinInfo != NULL)
	{
...
//
//初期化
HRESULT CD3DXSKINMESH::Init(HWND hWnd, ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	m_hWnd = hWnd;
	m_pDevice = pDevice;
	...
	//ブロブからバーテックスシェーダー作成
	if (FAILED(D3DX11CompileFromFile(L"./Mesh/Geometry_Material_Texture_Skin.hlsl", NULL, NULL, "VSSkin", "vs_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
	...
	//ブロブからピクセルシェーダー作成
	if (FAILED(D3DX11CompileFromFile(L"./Mesh/Geometry_Material_Texture_Skin.hlsl", NULL, NULL, "PSSkin", "ps_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
		MessageBox(0, L"hlsl読み込み失敗", NULL, MB_OK);
		return E_FAIL;
	}
...
HRESULT CD3DXSKINMESH::CreateFromX(CHAR* szFileName)
{
	...
	for (int i = 0; i < m_dwNumMaterial; i++)
	{
		...
		if (name)
		{
			...
			strcat(m_pMaterial[i].szTextureName, name);
...
void CD3DXSKINMESH::SetCamera(D3DXMATRIX view, D3DXMATRIX proj)
{
	m_View = view;
	m_Proj = proj;
}