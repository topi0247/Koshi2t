...
void Title_Scene::Init()
{
	...
	mesh_sword_ = creator->GetCharaModel("åïém");
	mesh_shiled_ = creator->GetCharaModel("èÇém");
	mesh_bomber_ = creator->GetCharaModel("îöíeém");
	mesh_witch_ = creator->GetCharaModel("ñÇì±ém");
	mesh_princess_ = creator->GetCharaModel("ïP");
	mesh_enemy_ = creator->GetCharaModel("ÉXÉâÉCÉÄ");
	mesh_stage_ = creator->GetStageModel("É^ÉCÉgÉã");
